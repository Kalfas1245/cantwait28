...
class HomeCubit extends Cubit<HomeState> {
  HomeCubit(this._itemRespository) : super(const HomeState());
  ...
  Future<void> start() async {
    _streamSubscription = _itemRespository.getItemsStream().listen(
      (items) {
  ...
  Future<void> remove({required String documentID}) async {
    try {
      await _itemRespository.delete(id: documentID);
    } catch (error) {
      emit(
